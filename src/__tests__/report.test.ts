...
import { is429, callLlm, saveFile, autoGenFooter } from "../reports/report.ts";
...
describe("is429", () => {
  ...
  it("detects Anthropic SDK APIError shape (status + headers)", () => {
    const sdkError = Object.assign(new Error("rate_limit_error"), {
      status: 429,
      headers: { "retry-after": "30" },
    });
    expect(is429(sdkError)).toBe(true);
  });
});
...
describe("saveFile", () => {
  ...
  it("returns the expected file path", () => {
    const result = saveFile("content", "2026-03-09", "news-tech.md");
    expect(result).toBe(path.join("digests", "2026-03-09", "news-tech.md"));
  });

  it("creates parent directories recursively", () => {
    saveFile("content", "2026-03-09", "news-tech.md");
...
describe("autoGenFooter", () => {
  it("returns a local Chinese footer", () => {
    expect(autoGenFooter("zh")).toContain("本地 News Radar");
  });

  it("returns a local English footer", () => {
    expect(autoGenFooter("en")).toContain("Generated locally by News Radar");
  });
});
